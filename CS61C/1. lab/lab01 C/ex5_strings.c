...
int main() {
  ...
  char* ptr_to_r = world_str[0];
  printf("address of 'r': %p\n", ptr_to_r);

  // TODO: create space to store the string "hello world"
  char* hello_world_str = malloc(sizeof(hello_str));
  if (hello_world_str == NULL) {
	  printf(stderr,"Memory allocation failed", strerror(errno));
	  return 1;
  }
  ...
  char* newSpace = realloc(hello_world_str, sizeof(strlen(hello_str) + 1 + strlen(world_str) + 1));
  if (newSpace == NULL) {
	  printf(stderr, "Memory allocation failed", strerror(errno));
	  return 1;
  }
  strcpy(newSpace, hello_world_str);
  hello_world_str = newSpace;
